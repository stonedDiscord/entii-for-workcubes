...
// We wrap FATFS around the raw block IO driver.
// We expose FS images on SD card/IDE hd as raw disks.
...
// 0-3 => EXI, 4 => IOSSDMC
#define DEVNAME_ENVIRONMENT "\\Device\\ArcEnviron%d"

#define SIGNATURE_PARTITION 'PART'
#define SIGNATURE_DISK 'DISK'

// EXI controller ARC path.
// Must be synchronised with arcfw\source\arcdisk.c
static const char s_ExiControllerPath[] = "multi(0)";

static PCONTROLLER_OBJECT s_ControllerObject[3] = { NULL, NULL, NULL };
...
static void SdmcEmuFinishDispatch(
	...
	if (Inserted) return;
	KeBugCheckEx(NO_MORE_IRP_STACK_LOCATIONS, Dpc, 0, 0, 'EXIB');
#endif
...
static ULONG DeviceToChannel(ULONG drive) {
	// 0,1,0,2
	if ((drive & 1) == 0) return 0;
	return (drive + 1) / 2;
}
...
static NTSTATUS SdmcEmuDeviceEnvCreate(
	...
	ULONG ExiIndex = DeviceToChannel(DeviceIndex);
	if (s_ControllerObject[ExiIndex] == NULL) {
		s_ControllerObject[ExiIndex] = IoCreateController(0);
		if (s_ControllerObject[ExiIndex] == NULL) return STATUS_INSUFFICIENT_RESOURCES;
	}
	...
	do {
		...
		Ext->DiskNumber = -1;
		Ext->DiskController = s_ControllerObject[ExiIndex];
...
static NTSTATUS SdmcEmuDeviceCreateImpl(
	...
	ULONG ExiIndex = DeviceToChannel(DriveIndex);
	if (s_ControllerObject[ExiIndex] == NULL) {
		s_ControllerObject[ExiIndex] = IoCreateController(0);
		if (s_ControllerObject[ExiIndex] == NULL) return STATUS_INSUFFICIENT_RESOURCES;
	}
	...
	do {
		...
		if (fr != FR_OK) {
			Ext->FfsFile.cltbl = NULL;
			Status = FresultToStatus(fr);
			break;
		}
...
NTSTATUS SdmcDiskRw(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
	...
	if (Ext->Signature == SIGNATURE_PARTITION) {
		...
		if (OffsetEnd.QuadPart > PartitionLength.QuadPart) {
			...
			if (Stack->MajorFunction == IRP_MJ_WRITE) {
				...
				OriginalOffset.QuadPart = Offset.QuadPart - Partition->StartingOffset.QuadPart;
				CHAR DebugText[512];
				_snprintf(DebugText, sizeof(DebugText), "EXIBLK: Tried to write partition %08x%08x-%08x%08x, past %08x%08x\r\n",
					OriginalOffset.HighPart, OriginalOffset.LowPart, OffsetEnd.HighPart, OffsetEnd.LowPart, PartitionLength.HighPart, PartitionLength.QuadPart
				);
				DbgPrint("%s", DebugText);
			}
#endif
			Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;
			IoCompleteRequest(Irp, IO_NO_INCREMENT);
	...
	if (
		...
		if (Stack->MajorFunction == IRP_MJ_WRITE) {
			CHAR DebugText[512];
			_snprintf(DebugText, sizeof(DebugText), "EXIBLK: Tried to write %08x%08x-%08x%08x, past %08x%08x\r\n",
				Offset.HighPart, Offset.LowPart, OffsetEnd.HighPart, OffsetEnd.LowPart, (ULONG)(DiskLength >> 32), (ULONG)DiskLength
			);
			DbgPrint("%s", DebugText);
		}
#endif
...
void SdmcDiskStartIo(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
	...
	//SdmcDiskSeizedController(DeviceObject, Irp, NULL, Ext);
}
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
	...
	//if (!NT_SUCCESS(ExiMapInit())) return STATUS_NO_SUCH_DEVICE;
	SdmcexiInit();
	IdeexiInit();
	...
	UCHAR InitedDevices = 0;
	for (ULONG i = 0; i < 4; i++) {
		if ((ExiDevices & BIT(i)) == 0) continue;
		if ((ExiDevices & BIT(i + 4)) != 0) {
			// IDE device.
			if (IdeexiMount(i)) InitedDevices |= BIT(i);
			else {
				_snprintf(Buffer, sizeof(Buffer), "EXIBLK: IdeexiMount(%d) failed\n", i);
				HalDisplayString(Buffer);
				continue;
			}
		} else {
			// SDMC device.
			if (SdmcexiMount(i)) InitedDevices |= BIT(i);
			else {
				_snprintf(Buffer, sizeof(Buffer), "EXIBLK: SdmcexiMount(%d) failed\n", i);
				HalDisplayString(Buffer);
				continue;
			}
		}
	...
	BOOLEAN MountedAtLeastOneDevice = FALSE;
	for (ULONG i = 0; i < 4; i++) {
		if ((InitedDevices & BIT(i)) == 0) continue;
		// Mount the exi device
		char path[4] = { '0' + i, ':', 0, 0 };
		fr = f_mount(&s_FatFs[i], path, 1);
		if (fr != FR_OK) {
			InitedDevices &= ~BIT(i);
			_snprintf(Buffer, sizeof(Buffer), "EXIBLK: f_mount(%d) failed %d\n", i, fr);
			HalDisplayString(Buffer);
			continue;
		}
		MountedAtLeastOneDevice = TRUE;
	}
	...
	for (ULONG drive = 0; drive < 4; drive++) {
		if ((InitedDevices & BIT(drive)) == 0) continue;
		...
		if ((ULONG)RUNTIME_BLOCK[RUNTIME_ENV_DISK] == drive) {
			sprintf(FileName, "%d:/" FILENAME_ENVIRONMENT, drive);
			// Firmware used environment from this sd card.
			fr = f_open(&fp, FileName, FA_READ | FA_WRITE);
			if (fr == FR_OK) {
				NTSTATUS Status = SdmcEmuDeviceEnvCreate(DriverObject, &fp, drive);
				if (NT_SUCCESS(Status)) {
					OpenedAtLeastOneDevice = TRUE;
				} else {
					_snprintf(Buffer, sizeof(Buffer), "EXIBLK: SdmcEmuDeviceEnvCreate(%d) failed %08x\n", drive, Status);
					HalDisplayString(Buffer);
				}
			} else {
				_snprintf(Buffer, sizeof(Buffer), "EXIBLK: f_open(%d:environ) failed %d\n", drive, fr);
				HalDisplayString(Buffer);
			}
		}
		
		RtlZeroMemory(&fp, sizeof(fp));
		sprintf(FileName, "%d:/" FILENAME_FLOPPY_IMAGE, drive);
		fr = f_open(&fp, FileName, FA_READ);
		if (fr == FR_OK) {
			// Got a floppy drive.
			if (NT_SUCCESS(
				SdmcEmuFloppyCreate(DriverObject, &fp, &Config->FloppyCount, drive)
			)) {
				OpenedAtLeastOneDevice = TRUE;
			}
		// Do not print an error if that error was file not found.
		} else if (fr != FR_NO_FILE) {
			_snprintf(Buffer, sizeof(Buffer), "EXIBLK: f_open(%d:floppy) failed %d\n", drive, fr);
			HalDisplayString(Buffer);
		}
		...
		for (ULONG i = 0; i < 100; i++) {
			RtlZeroMemory(&fp, sizeof(fp));
			sprintf(FileName, "%d:/" FILENAME_CDROM_IMAGE, drive, i);
			fr = f_open(&fp, FileName, FA_READ | FA_WRITE);
			if (fr != FR_OK) {
				// Do not print an error if that error was file not found.
				if (fr == FR_NO_FILE) break;
				_snprintf(Buffer, sizeof(Buffer), "EXIBLK: f_open(%d:cd%d) failed %d\n", drive, i, fr);
				HalDisplayString(Buffer);
				break;
			}
			// Got an ISO image.
			if (NT_SUCCESS(
				SdmcEmuCdromCreate(DriverObject, &fp, &Config->CdRomCount, drive, i)
			)) {
				OpenedAtLeastOneDevice = TRUE;
			}
		}
		...
		for (ULONG i = 0; i < 100; i++) {
			sprintf(FileName, "%d:/" FILENAME_HARD_DISK_IMAGE, drive, i);
			RtlZeroMemory(&fp, sizeof(fp));
			fr = f_open(&fp, FileName, FA_READ | FA_WRITE);
			if (fr != FR_OK) {
				// Do not print an error if that error was file not found.
				if (fr == FR_NO_FILE) break;
				_snprintf(Buffer, sizeof(Buffer), "EXIBLK: f_open(%d:hd%d) failed %d\n", drive, i, fr);
				HalDisplayString(Buffer);
				break;
			}
			...
			if (NT_SUCCESS(
				SdmcEmuDiskCreate(DriverObject, &fp, &Config->DiskCount, drive, i)
			)) {
				OpenedAtLeastOneDevice = TRUE;
			}
		}
